...
export default async function Captures() {
  ...
  try { captures = await listCaptures(); } catch (error) {
    if (error instanceof CaptureError && error.status === 401) return <section className="panel"><h1>Your capture inbox</h1><p>Sign in to keep your thoughts across days.</p><Link href="/app/you">Your account →</Link></section>;
    throw error;
  }
