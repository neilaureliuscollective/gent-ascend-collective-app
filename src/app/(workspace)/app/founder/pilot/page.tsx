...
export default async function PilotConsole({ searchParams }: {
  ...
  if (!cohort) return <section className="panel pilot-section"><p className="eyebrow">Private founder space</p><h1>Founder access required.</h1><p>Sign in with your founder account to manage the pilot.</p><Link className="button" href="/app/you">Go to your account</Link></section>;
  const { status } = await searchParams;
  return <>
    <div className="page-heading compact-heading"><div><p className="eyebrow">Founder / Private pilot</p><h1>Founding members.</h1></div><Link className="text-link" href="/app/welcome">View arrival →</Link></div>
