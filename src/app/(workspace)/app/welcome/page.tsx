...
export default async function Welcome({ searchParams }: {searchParams: Promise<{status?: string}>}) {
  ...
  return <>
    ...
    {!pilot ? <section className="panel pilot-section"><h2>Accept your invitation</h2><p>Open the invitation sent to your email. If you already have an account, sign in with that email to continue.</p><Link className="button" href="/app/you">Go to sign in</Link></section> : <>
      {!pilot.beta && !pilot.founder ? <section className="panel pilot-section"><h2>Confirm your place</h2><p>Your invitation is tied to your verified email. Confirm it here to open the founding member experience.</p><form action={claimPilotAction}><button className="button">Confirm my access</button></form></section> : <>
        <div className="personal-grid"><section className="panel"><p className="eyebrow">Your first session</p><h2>Begin with one clear step.</h2><ol className="pilot-steps">
          <li><Link href="/app/you">Set your priority</Link><p>Name what matters now and set your timezone.</p></li>
          <li><Link href="/app/ascend-profile">Build your baseline</Link><p>Keep only the facts you choose to confirm.</p></li>
          <li><Link href="/app/goals">Choose one goal</Link><p>Give it a next action you can actually take.</p></li>
          <li><Link href="/app/install">Install on your phone</Link><p>Add Gent Ascend to your Home Screen, then reopen it from the icon.</p></li>
          <li><Link href="/app">Use Command today</Link><p>Write your intention, act, then return for an evening review.</p></li>
        </ol><Link className="button" href="/app">Open Command</Link></section>
        <aside className="panel perspective-panel"><p className="eyebrow">Your personal intelligence</p><h2>Talk it through.</h2><p>Aethelios can help you think, plan and reflect. Your private work remains yours; a proposal only changes your daily plan when you confirm it.</p><Link className="text-link" href="/app/aethelios">Meet Aethelios →</Link></aside></div>
