...
export async function claimPilotAction() {
  ...
  redirect(`/app/welcome?status=${status}`);
}

export async function setPilotPassword(form: FormData) {
  const parsed = z.string().min(12).max(128).safeParse(form.get('password'));
  if (!parsed.success) redirect('/app/welcome?status=password-invalid');
  const client = await serverClient();
  if (!client || (await client.auth.getUser()).error) redirect('/app/welcome?status=auth');
  const { error } = await client.auth.updateUser({ password: parsed.data });
  redirect(`/app/welcome?status=${error ? 'password-error' : 'password-saved'}`);
}
...
export async function submitFeedbackAction(form: FormData) {
  ...
  if (!parsed.success) redirect('/app/welcome?status=feedback-invalid');
  try { await submitPilotFeedback(parsed.data.category, parsed.data.message); }
  catch { redirect('/app/welcome?status=feedback-error'); }
  revalidatePath('/app/founder/pilot');
  redirect('/app/welcome?status=feedback-saved');
}
