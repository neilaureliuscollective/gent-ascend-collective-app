...
2. Put AI_GATEWAY_API_KEY in ignored .env.local, using a dedicated Gateway key with a spend budget. AURELIUS_AI_MODEL optionally overrides the verified default openai/gpt-6-astra. Do not put the key in client code or chat. dev:setup rewrites .env.development.local, so keep AI settings separate.
